function [state, traj] = stateManager(position, platform_position, detected, t)
    ...
    persistent current_state hover_start_time last_t position_error_start_time detection_start_time
    % NEW: Variables for State 3 smoothing
    persistent state3_start_time state3_start_pos
    ...
    if isempty(current_state)
        ...
        % NEW: Initialize State 3 variables
        state3_start_time = -1;
    ...
    switch current_state
        ...
        case 3  % Go on a circular trajectory while trying to find the moving platform
            ...
            % 1. Calculate the IDEAL (Target) Circular Trajectory
            center = [0, 0];
            radius = 6.0;
            ...
            % 2. Initialize Transition Logic if just entered State 3
            if state3_start_time < 0
                state3_start_time = t;
                state3_start_pos  = position; % Snapshot current actual position (e.g., [0,0,2.5])
            end

            % 3. Apply Smoothing Logic (First 2 seconds)
            T_trans = 3.0; % Transition duration
